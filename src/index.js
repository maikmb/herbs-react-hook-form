...
export const herbsValidationResolver =
  (entity) =>
  (values, _context, { names }) => {
    ...
    if (names) {
      const invalidKeys = names.filter((key) =>
        Object.keys(instanceErrors).includes(key)
      )
      errors = invalidKeys.reduce(
        (err, key) => ({
          ...err,
          [key]: {
            type: `min-length`,
      ...
      errors = Object.keys(instanceErrors).reduce(
        (err, key) => ({
          ...err,
          [key]: {
            type: `min-length`,
